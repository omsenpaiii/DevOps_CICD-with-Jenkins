pipeline {
    ...
    stages {
        stage('Checkout git') {
            steps {
              git branch: 'main', url:'https://github.com/omsenpaiii/DevOps_CICD-with-Jenkins.git'
            }
        }
            ...
            steps {
                script {
                    if (fileExists('DevOps_MasterPiece-CD-with-argocd')) {
                        ...
                        sh 'git clone -b feature https://github.com/omsenpaiii/DevOps_CICD-with-Jenkins.git'
                    }
                }
            }
        }
        ...
        stage('Commit & Push changes to feature branch') {
            steps {
                withCredentials([string(credentialsId: 'GITHUB_TOKEN', variable: 'GITHUB_TOKEN')]) {
                    dir("DevOps_MasterPiece-CD-with-argocd/yamls") {
                        sh "git config --global user.email 'omtomar4882.be22@chitkara.edu.in'"
